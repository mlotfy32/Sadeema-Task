...
class CashcourcesCubit extends Cubit<CashcourcesState> {
  ...
  getCources() async {
    ...
    Box<CashCource> courcesBox = await Hive.box(PrefsKeys.hiveCources);
    // courcesBox.clear();
    try {
  ...
  addCources({
    ...
    SharedPref.preferences.setLsit(PrefsKeys.courcesChasedName, oldList);
    Box<CashCource> courcesBox = await Hive.box(PrefsKeys.hiveCources);
  ...
  updateCources({
    ...
    var courcesBox = Hive.box<CashCource>(PrefsKeys.hiveCources);
